...
export function buildDocumentAnalysisPrompt(input: DocumentAnalysisPromptInput): string {
  ...
  return [
    ...
    "",
    "UNCERTAINTIES array items must be objects (not strings) using exactly these fields:",
    "code (short machine-readable string, e.g. \"illegible_dose\"), message (patient-friendly explanation of the uncertainty),",
    "relatedField (string naming the field this refers to, e.g. \"medicines[0].strengthAsWritten\", or null if general).",
    "",
    "SAFETYNOTES array items must be objects (not strings) using exactly these fields:",
    "code (short machine-readable string, e.g. \"not_a_diagnosis\"), message (the note text shown to the patient),",
    "severity (must be exactly \"info\" or \"warning\").",
    "",
