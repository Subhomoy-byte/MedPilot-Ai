...
export const CopilotChat: React.FC<CopilotChatProps> = ({ document }) => {
  ...
  const handleSend = (questionText?: string) => {
    ...
    // Simulate intelligent context-aware response based on document
    setTimeout(() => {
      let botResponse = '';
      const qLower = query.toLowerCase();

      if (qLower.includes('food') || qLower.includes('meal')) {
        const medsWithFood = document.medicines.map((m) => `${m.name} (${m.withFood.replace('_', ' ')})`).join(', ');
        botResponse = `Based on this prescription, here are your food timing rules:\n• ${medsWithFood}.\n\nTaking medications like Metformin with a meal significantly reduces stomach upset.`;
      } else if (qLower.includes('high') || qLower.includes('lab') || qLower.includes('test')) {
        const highLabs = document.labs.filter((l) => l.status === 'high');
        if (highLabs.length > 0) {
          botResponse = `Your report highlights ${highLabs.map((l) => l.name).join(', ')} as elevated above standard reference ranges. ${highLabs[0].plainExplanation}`;
        } else {
          botResponse = `All lab values in this record are currently within standard ranges.`;
        }
      } else if (qLower.includes('ask') || qLower.includes('doctor') || qLower.includes('pharmacist')) {
        botResponse = `Here is the top question recommended for your clinician: "${document.suggestedQuestions[0] || 'Could you clarify the exact daily schedule for my medications?'}"`;
      } else {
        botResponse = `In this document (${document.title}), there are ${document.medicines.length} prescribed medications and ${document.labs.length} lab tests. Feel free to ask about specific medicine instructions, safety precautions, or lab reference ranges.`;
      }
